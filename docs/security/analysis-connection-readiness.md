...
재조회와 다른 운영 트랜잭션을 전역으로 직렬화하거나 이미 예약된 PROMOTE 명령을 취소하는 기능은 아니다. Worker 호출 직전에 확인한 후 발생하는 비활성화는 진행 중 네트워크 요청 자체를 취소하지 않는다. 동일 연결 ID의 모든 설정 변경에 대한 별도 세대 snapshot을 제공하지 않는다. 실제 운영 연결을 비활성화하지 않았다. 초기 코드 보강 이후 [v0.52.0 배포 검증](../operations/analysis-safety-v0.52.0.md)에서 공개 이미지 반영까지 확인했다.
