...
98. Admission webhook Pod 장애 내구성 — 완료 (2026-09-16, replica 2/failurePolicy Fail/PDB minAvailable 1 Helm values 영속화·실제 webhook Pod 강제 삭제 중 현재 서명 digest server-side admission 성공·대체 replica 복구·정적 fail-closed/중복/PDB 회귀 포함 23개; 현재 단일 EKS 노드에 두 Pod가 함께 있어 노드/AZ 장애 내구성은 미확보, required 분산은 추가 노드 비용·용량 검토 후 별도)
