...
192. 감사 증거 무결성 JSON 보고서 내보내기 — 진행 중 (2026-09-19, 브라우저 안에서만 생성되는 `releasepilot.evidence-integrity/v1` JSON 보고서에 SHA-256/GENESIS/검증 결과/최초 불일치 ID/무효 링크 수와 6개 원본 레코드·전체 expected/actual hash·레코드별 유효성을 포함·정상/변조 탐지 결과별 파일명 분리·네트워크 전송 없이 Blob 다운로드·다운로드 파일 내용/4개 무효 레코드 회귀 검사·단위 38개/lint/typecheck/build·쇼케이스 Chromium 11개/전체 Chromium 224개/WebKit 접근성 3개 통과·변조 실패 화면 시각 검수)
