...
데모 로그인 POST 실패 후속 검증: 첫 로그인 POST를 연결 실패 또는 200/깨진 JSON으로 응답한다. 오류 표시·버튼 복구·DEMO 상태 없음·처리되지 않은 pageerror 없음, 정상 fixture 응답으로 수동 재시도 후 DEMO 상태·이전 오류 제거·POST 총 2회가 확인됐다. 신규 2개 포함 전체 fixture E2E 88개 및 lint/typecheck가 통과했다. 제품 코드 변경은 없다. 문법상 유효하지만 세션 구조가 잘못된 응답, 실제 서버 세션 생성 후 응답 유실은 별도다.
