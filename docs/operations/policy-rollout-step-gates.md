...
Promotion requires a COMPLETED PASS analysis for the current policy step. The HTTP operation rejects premature manual promotion with `ROLLOUT_ANALYSIS_PASS_REQUIRED`; the command handler checks again before any Kubernetes control patch, including automatic actions. Healthy native state alone cannot mark the release successful: all policy steps must have passed.
