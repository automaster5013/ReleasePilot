...
public class DefaultEnvironmentInspector implements EnvironmentInspector {
 ...
 public List<Check> inspect(Environment env){
  ...
  boolean strategy=rollout&&env.getRolloutStrategy()==k.strategy();
  ...
  boolean promReachable=p.ready()&&p.queryReachable(); boolean policyOk=policy.exists()&&policy.active()&&policy.semanticallyValid()&&policy.strategy()==env.getRolloutStrategy();
  return List.of(
   ...
   check("ROLLOUT_STRATEGY",strategy,"Rollout strategy matches "+env.getRolloutStrategy(),"Rollout strategy does not match "+env.getRolloutStrategy(),json("observedStrategy",k.strategy()==null?"":k.strategy().name())),
   check("TRAFFIC_SERVICES_MATCH",traffic,"Traffic service references match","Rollout traffic service references do not match","{}"),
   check("KUBERNETES_SERVICES_EXIST",services,"Both Kubernetes Services exist","A required Kubernetes Service is missing","{}"),
   ...
   check("DEFAULT_POLICY_ACTIVE",policyOk,"Default policy is active, valid, and strategy-compatible","Default policy is missing, inactive, invalid, or uses another strategy","{}"));
 }
