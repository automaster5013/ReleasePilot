...
public class PolicyDefinitionValidator {
 ...
 public Result validate(String definition){
  ...
  try{JsonNode root=json.readTree(definition);
   ...
   Map<String,Integer> counts=new HashMap<>();Set<String> scopes=new HashSet<>();JsonNode metrics=root.path("metrics");if(!metrics.isArray())errors.add("metrics must be an array");else for(JsonNode metric:metrics){String key=metric.path("key").asText(),route=metric.path("route").asText("");if(route.isBlank())counts.merge(key,1,Integer::sum);String scope=key+"@"+route;if(!scopes.add(scope))errors.add("Metric key and route must be unique: "+scope);if(!route.isBlank()&&!route.matches("^/[-A-Za-z0-9._~/{}]+$"))errors.add("route must be a safe normalized HTTP route template");String importance=metric.path("importance").asText("STANDARD");if(!Set.of("STANDARD","CRITICAL").contains(importance))errors.add("importance must be STANDARD or CRITICAL");if("CRITICAL".equals(importance)&&(route.isBlank()||!metric.path("required").asBoolean(false)))errors.add("CRITICAL route metrics must define route and be required");if(REQUIRED.contains(key)&&route.isBlank()&&!metric.path("required").asBoolean(false))errors.add(key+" must be required");if("HTTP_5XX_RATE".equals(key)){double threshold=metric.path("threshold").asDouble(-1);if(threshold<0||threshold>1)errors.add("HTTP_5XX_RATE threshold must be between 0 and 1");}if("REQUEST_COUNT".equals(key)&&!metric.path("relativeToBaseline").isMissingNode())errors.add("REQUEST_COUNT cannot define relativeToBaseline");}
