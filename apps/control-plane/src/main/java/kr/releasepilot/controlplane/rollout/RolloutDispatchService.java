...
public class RolloutDispatchService {
    ...
    public RolloutExecution schedule(Release release, Environment environment, Instant now) {
        ...
        try{var policy=json.readTree(snapshots.findByReleaseId(release.getId()).orElseThrow().getDefinition());var definitions=policy.path("steps");if(!definitions.isArray()||definitions.isEmpty())throw new IllegalStateException("Policy snapshot has no Canary steps");int previous=0;for(int index=0;index<definitions.size();index++){var definition=definitions.get(index);int weight=definition.path("weight").asInt();int observation=definition.path("minimumObservationSeconds").asInt(-1);if(weight<=previous)throw new IllegalStateException("Canary weights must increase");steps.save(RolloutStep.pending(execution.getId(),index,weight,observation));previous=weight;}if(previous!=100)throw new IllegalStateException("Last Canary weight must be 100");}catch(RuntimeException e){throw e;}catch(Exception e){throw new IllegalStateException("Cannot create Rollout steps from policy snapshot",e);}
