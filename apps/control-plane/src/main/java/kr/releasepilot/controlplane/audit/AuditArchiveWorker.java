...
class AuditArchiveWorker {
    ...
    public void tick(){for(var delivery:deliveries.findByStatusAndAvailableAtLessThanEqualOrderByAvailableAtAsc(AuditArchiveDelivery.Status.PENDING,clock.instant(),PageRequest.of(0,20))){try{sink.archive(events.findById(delivery.auditEventId()).orElseThrow());delivery.delivered(clock.instant());}catch(Exception failure){delivery.failed(failure.getMessage()==null?failure.getClass().getSimpleName():failure.getMessage(),clock.instant());}}}
}
